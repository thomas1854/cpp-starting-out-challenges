...
private:
    ...
    int* widths;           // Dynamically allocated array that stores the number of nodes at each tree level
    ...
    int subtreeHeight(TreeNode*) const;           // Helper function: recursively computes the height of the subtree rooted at the given node
    void getWidths(TreeNode*, int);               // Helper function that recursively counts nodes at each tree level
public:
    ...
    // Public method: computes the height of the tree by calling the recursive helper on the root
    int getHeight() const 
    {
...
// Private recursive helper that computes the height of a subtree.
// The height is defined as the number of nodes on the longest path 
// from the given node down to a leaf.
template<typename T>
int BinaryTree<T>::subtreeHeight(TreeNode* nodePtr) const
{
    ...
    int leftSubtreeHeight = subtreeHeight(nodePtr->left);

    // Recursively compute the height of the right subtree
    int rightSubtreeHeight = subtreeHeight(nodePtr->right);

    // Height of the current subtree is 1 (for the current node)
    // plus the larger of the two subtree heights
    return 1 + std::max(leftSubtreeHeight, rightSubtreeHeight);
}

// Recursively count nodes at each level
template<typename T>
void BinaryTree<T>::getWidths(TreeNode* nodePtr, int level)
{
    if (nodePtr != nullptr)
    {
        widths[level]++;                          // Increment count for this level
        getWidths(nodePtr->left, level + 1);      // Recurse left subtree
        getWidths(nodePtr->right, level + 1);     // Recurse right subtree
    }
}

// Return the maximum number of nodes on the same level (tree width)
template<typename T>
int BinaryTree<T>::getTreeWidth()
{
    if (widths != nullptr)                        // Free old memory if already allocated
        delete[] widths;

    int treeHeight = getHeight();                 // Get the height of the tree
    widths = new int[treeHeight + 1]();           // Allocate and initialize counts to zero

    getWidths(root, 1);                           // Fill widths array by traversing tree

    int width = 0;
    for (int i = 1; i <= treeHeight; i++)         // Find the maximum count across levels
        width = std::max(width, widths[i]);
