...
public:
    ...
    T searchNode(T) const;          // Searches for a value in the tree
...
// Searches for a value in the tree; returns the object if found, otherwise returns default object of type T
template<typename T>
T BinaryTree<T>::searchNode(T value) const {
    ...
    while (nodePtr)
    {
        if (nodePtr->value == value)
            return nodePtr->value; // Object found - return the actual object
        else if (nodePtr->value < value)
            nodePtr = nodePtr->right; // Search right subtree (value is greater)
        else
            nodePtr = nodePtr->left;  // Search left subtree (value is smaller)
    }
    
    return T(); // Object not found - return default constructed object of type T
}
